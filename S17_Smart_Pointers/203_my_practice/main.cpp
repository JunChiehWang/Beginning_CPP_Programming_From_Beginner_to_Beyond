...
// functions: make
std::unique_ptr<std::vector<std::shared_ptr<Test>>> make() {
    std::unique_ptr<std::vector<std::shared_ptr<Test>>> my_ptr = std::make_unique<std::vector<std::shared_ptr<Test>>>();    
    return my_ptr;
};
// function: fill
void fill(std::vector<std::shared_ptr<Test>> &vec, int num) {
    for (size_t i {1}; i< num+1; i++) {
        std::cout << "Enter int data point [" << i << "] :";
        int num_tmp;
        std::cin >> num_tmp;
        std::shared_ptr<Test> ptr = std::make_shared<Test> (num_tmp); 
        vec.push_back(ptr);
    }
};
// function: disdplay
